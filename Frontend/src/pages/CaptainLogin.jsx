...
const CaptainLogin = () => {
  ...
  const submitHandler =async (e) => {
    ...
    const captain={
     ...
     const respone=await axios.post(`${import.meta.env.VITE_BASE_URL}/captains/login`,captain)
    ...
    <div className='p-7 h-screen flex flex-col justify-between'>
      <div>
        ...
        <form onSubmit={(e) => {
          ...
          <p className='text-center mb-4'>Join a fleet?
            <Link to='/captain-signup' className='text-blue-600'>Regitser as a Captain</Link>  </p>
      ...
      <div>
        <Link
          to='/login'
          className='bg-[#d5622d] flex justify-center items-center text-white font-semibold mb-7 rounded px-4 py-2 border w-full text-lg '>Sign in as User</Link>
      </div>
    </div>
  )
}
